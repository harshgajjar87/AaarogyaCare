...
const DoctorAppointments = () => {
  ...
  const StatCard = ({ title, value, color }) => (
    <div className={`bg-white rounded-lg sm:rounded-xl shadow-sm border-l-4 border-${color}-500 p-3 sm:p-4 md:p-6`}>
      <h5 className="text-slate-500 text-xs sm:text-sm">{title}</h5>
      <p className="text-slate-900 text-xl sm:text-2xl md:text-3xl font-bold">{value}</p>
    </div>
  );
  ...
  return (
    <div className="space-y-4 sm:space-y-6 md:space-y-8 px-3 sm:px-4 md:px-6">
      <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
        ...
        <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-health-text-h">Appointments</h1>
      </div>
      ...
      <div className="bg-white rounded-lg sm:rounded-xl shadow-sm border p-3 sm:p-4 md:p-6">
        <div className="flex flex-col md:flex-row justify-between gap-3 sm:gap-4 mb-3 sm:mb-4">
          <div className="relative w-full md:w-1/2">
            <Search className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
            <input
              type="text"
              className="w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-1.5 sm:py-2 border border-slate-300 rounded-full text-sm sm:text-base"
          ...
          <div className="flex gap-2 sm:gap-3 md:gap-4">
            <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="flex-1 md:flex-none border border-slate-300 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm">
            ...
            <button onClick={exportDoctorAppointments} className="bg-teal-600 text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs sm:text-sm text-left text-slate-600">
            <thead className="text-[10px] sm:text-xs text-slate-700 uppercase bg-slate-50">
              <tr>
                ...
                <th className="px-2 sm:px-4 md:px-6 py-2 sm:py-3 text-center">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredAppointments.map(a => (
                <tr key={a._id} className="bg-white border-b hover:bg-slate-50">
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 font-medium text-slate-900 text-[10px] sm:text-xs md:text-sm">{a.patientId?.name || 'N/A'}</td>
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 text-[10px] sm:text-xs md:text-sm">{new Date(a.date).toLocaleDateString()} at {a.time}</td>
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4">
                    <span className={`px-1.5 sm:px-2 py-0.5 sm:py-1 font-semibold leading-tight rounded-full text-[10px] sm:text-xs ${getStatusClass(a.status)}`}>
                      {a.status.replace('-by-patient', '')}
                    </span>
                  </td>
                  <td className="px-2 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 text-center">
                    {a.status === 'approved' && (
                      <button onClick={() => handleChatWithPatient(a._id)} className="bg-blue-500 text-white px-2 sm:px-3 py-1 rounded-full flex items-center gap-1 text-[10px] sm:text-xs mx-auto">
                        <MessageSquare size={12} className="sm:w-3.5 sm:h-3.5" /> <span className="hidden sm:inline">Chat</span>
                      </button>
                    )}
                  </td>
                </tr>
          ...
          {filteredAppointments.length === 0 && <p className="text-center py-6 sm:py-8 text-sm sm:text-base">No appointments found.</p>}
        </div>
      </div>
    </div>
  );
